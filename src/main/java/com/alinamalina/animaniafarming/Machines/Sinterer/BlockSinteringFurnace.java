...
public class BlockSinteringFurnace extends BlockBase implements ITileEntityProvider 
{
	...
	public BlockSinteringFurnace(String name, Material material)
	{
		super(name, material);
		setHardness(5.0F);
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitY, float hitX, float hitZ) //What happens when you right click the block
	{
		if(!worldIn.isRemote)
		{
			playerIn.openGui(Main.instance, GuiIDs.GUI_SINTERING_FURNACE, worldIn, pos.getX(), pos.getY(), pos.getZ());//Opens GUI of the furnace
		}
		return true;
	...
	{
		TileEntitySinteringFurnace te = (TileEntitySinteringFurnace)worldIn.getTileEntity(pos);
		InventoryHelper.spawnItemStack(worldIn, pos.getX(), pos.getY(), pos.getZ(), te.handler.getStackInSlot(0));
		InventoryHelper.spawnItemStack(worldIn, pos.getX(), pos.getY(), pos.getZ(), te.handler.getStackInSlot(1));
		InventoryHelper.spawnItemStack(worldIn, pos.getX(), pos.getY(), pos.getZ(), te.handler.getStackInSlot(2));
		InventoryHelper.spawnItemStack(worldIn, pos.getX(), pos.getY(), pos.getZ(), te.handler.getStackInSlot(3));
	...
	public static void setState(boolean active, World worldIn, BlockPos pos)
	{
		...
		if (active)
			worldIn.setBlockState(pos,  ModBlocks.SINTERING_FURNACE.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(BURNING, true), 3) ;//If active then burning
		else
			worldIn.setBlockState(pos, ModBlocks.SINTERING_FURNACE.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(BURNING, false), 3);//If not active then not burning
