...
public class ContainerCompostGenerator extends Container
{
	private final TileEntityCompostGenerator tileentity;
	private int energy, cookTime;
	
	public ContainerCompostGenerator(InventoryPlayer player, TileEntityCompostGenerator tileentity) 
	{
		this.tileentity = tileentity;
		IItemHandler handler = tileentity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null);
		
				this.addSlotToContainer(new SlotItemHandler(handler, 0, 52, 35));
	...
	public void detectAndSendChanges() 
	{
		...
		for(int i = 0; i < this.listeners.size(); ++i) 
		{
			...
			if(this.cookTime != this.tileentity.getField(1)) listener.sendWindowProperty(this, 1, this.tileentity.getField(2));
		}
